window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ── Style override: replace fast shimmer with slow breathing pulse ──

    var breathingKeyframes = null;

    function injectBreathingKeyframes() {
      if (breathingKeyframes) return;
      var style = document.createElement("style");
      style.textContent = [
        "@keyframes dsh-dynamic-pulse {",
        "  0%, 100% { opacity: 0.5; }",
        "  50%      { opacity: 1; }",
        "}",
      ].join("\n");
      document.head.appendChild(style);
      breathingKeyframes = style;
    }
    ...
    function takeoverTurnStatus(el) {
      injectBreathingKeyframes();
      if (el.dataset.dynamicStatusActive === "true") return;
      el.dataset.dynamicStatusActive = "true";

      // ── Override inline styles: kill shimmer, keep blue tone, add breathing ──
      ...
      el.style.setProperty("color", "rgb(120, 150, 220)", "important");
      el.style.setProperty("-webkit-text-fill-color", "rgb(120, 150, 220)", "important");
      el.style.setProperty("animation", "dsh-dynamic-pulse 4s ease-in-out infinite", "important");
      ...
      function render() {
        ...
        if (textNode) {
          textNode.textContent = frame + " " + current;
        }
      }
      ...
      // ── Fast spinner loop (200ms) ──────────────────────────────────
      (function spin() {
        if (!el.isConnected) return;
        render();
        spinnerTimer = setTimeout(spin, 200);
      })();
      ...
      function cycle() {
        if (!el.isConnected) return;
        current = pickMessage(current);
        // Don't render here — the spinner loop will pick it up next tick
      }

      function scheduleNext() {
        var delay = 10000 + Math.random() * 5000;  // 10–15s
      ...
      takeoverTimers.push(spinnerTimer);
    }
